...
public class HumanPyramid {
  ...
  public float totalWeightSupported(List<List<Double>> pyramid, int row, int col) {
    float ans = (float)solve(row, col, pyramid);
    return ans;
  }

  private double solve(int r, int c, List<List<Double>> pyramid) {
    if (r < 0 || c < 0 || c > r) {
        return 0;
    }

    if (r == 0 && c == 0) {
        return pyramid.get(r).get(c);
    }
    
    double ans = pyramid.get(r).get(c);
    ans += (solve(r - 1, c, pyramid) + solve(r - 1, c - 1, pyramid)) / 2.0;
